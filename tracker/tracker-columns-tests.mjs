 ...
 * SNIPE_TRACKER / SNIPE_ADDITIONS env overrides and assert:
...
function runScript(script, args, sandbox) {
  const env = {
    ...process.env,
    SNIPE_TRACKER: sandbox.tracker,
    SNIPE_ADDITIONS: sandbox.additions,
    SNIPE_TRACKER_LOCK: sandbox.lock,
  };
  try {
    const stdout = execFileSync(NODE, [join(SCRIPT_DIR, script), ...args], {
