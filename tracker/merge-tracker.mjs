 ...
 *
 * Run: node tracker/merge-tracker.mjs [--dry-run] [--verify]
 */
...
import { SNIPE_ROOT, trackerPath } from './paths.mjs';
const APPS_FILE_RAW = trackerPath();
const APPS_FILE = canonicalizeTrackerPath(APPS_FILE_RAW);
const TRACKER_DIR = dirname(APPS_FILE);
// SNIPE_ADDITIONS overrides the additions dir (used by tests, mirrors SNIPE_TRACKER).
const ADDITIONS_DIR = process.env.SNIPE_ADDITIONS
  ? process.env.SNIPE_ADDITIONS
  : join(SNIPE_ROOT, 'batch/tracker-additions');
const MERGED_DIR = join(ADDITIONS_DIR, 'merged');
const DRY_RUN = process.argv.includes('--dry-run');
const VERIFY = process.argv.includes('--verify');
const MIGRATE = process.argv.includes('--migrate');
const MERGE_HOLD_MS = Number(process.env.SNIPE_MERGE_HOLD_MS) || 0;
const MERGE_READY_IPC = process.env.SNIPE_MERGE_READY_IPC === '1';

const trackerLockKey = createHash('sha256').update(APPS_FILE).digest('hex').slice(0, 16);
const TRACKER_LOCK_DIR = resolveTrackerLockDir(process.env.SNIPE_TRACKER_LOCK, trackerLockKey);
...
mkdirSync(join(SNIPE_ROOT, 'data'), { recursive: true });
 ...
 *
 * `SNIPE_TRACKER_LOCK` exists for tests and unusual local layouts, but the
 ...
 * and use the snipe lock-name prefix. Invalid values are ignored and the
...
function resolveTrackerLockDir(envValue, lockKey) {
  ...
  const fallback = join(tmpRoot, `snipe-merge-tracker-${lockKey}.lock`);
  ...
  if (!basename(candidate).startsWith('snipe-merge-tracker-')) return fallback;
  return candidate;
}
 ...
 * - the regression test hook (`SNIPE_MERGE_HOLD_MS`), which deliberately
...
try {
  trackerLock = await acquireTrackerLock(TRACKER_LOCK_DIR, {
    timeoutMs: Number(process.env.SNIPE_TRACKER_LOCK_TIMEOUT_MS) || 60_000,
    retryMs: Number(process.env.SNIPE_TRACKER_LOCK_RETRY_MS) || 75,
    staleMs: Number(process.env.SNIPE_TRACKER_LOCK_STALE_MS) || 10 * 60_000,
  });
 ...
 *
 * Batch workers and hand edits may emit non-canonical variants, bold
...
// A literal `|` in any cell shifts every column after it and corrupts the row
// (e.g. a role named "Staff Software Engineer | Console"). Substituted, not
// backslash-escaped: both independent parsers (this file, verify-pipeline.mjs)
// split on a raw `|`, so `\|` would still split. ponytail: lossy by design — the
// table is a display view, and batch/evals/*.json keeps the verbatim role.
...
function parseTsvContent(content, filename) {
  ...
  if (content.startsWith('|')) {
    ...
    const statusRe = /^(evaluated|applied|responded|interview|offer|rejected|discarded|skip|dup|repost|hold|monitor|sent)/i;
    const col4LooksLikeStatus = statusRe.test(col4);
    const col5LooksLikeStatus = statusRe.test(col5);
...
if (VERIFY && !DRY_RUN) {
  ...
  try {
    execFileSync('node', [join(SNIPE_ROOT, 'tracker/verify-pipeline.mjs')], { stdio: 'inherit' });
  } catch (e) {
    process.exit(1);
  }
}
