#!/usr/bin/env node
/**
 * followup-cadence.mjs — Follow-up Cadence Tracker for snipe
 *
 ...
 *
 * Run: node tracker/followup-cadence.mjs             (JSON to stdout)
 *      node tracker/followup-cadence.mjs --summary   (human-readable dashboard)
 *      node tracker/followup-cadence.mjs --overdue-only
 *      node tracker/followup-cadence.mjs --applied-days 10
 */
...
const FOLLOWUPS_FILE = join(SNIPE_ROOT, 'data/follow-ups.md');
const PROFILE_FILE = process.env.SNIPE_PROFILE || join(SNIPE_ROOT, 'config/profile.yml');
...
function resolveReportPath(reportField) {
  ...
  const fullPath = join(SNIPE_ROOT, match[1]);
