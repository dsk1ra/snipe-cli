#!/usr/bin/env node
/**
 * verify-pipeline.mjs — Health check for snipe pipeline integrity
 *
 * Checks:
 ...
 *
 * Run: node tracker/verify-pipeline.mjs
 */
...
const APPS_FILE = trackerPath();
const ADDITIONS_DIR = join(SNIPE_ROOT, 'batch/tracker-additions');
const REPORTS_DIR = join(SNIPE_ROOT, 'reports');
const STATES_FILE = existsSync(join(SNIPE_ROOT, 'templates/states.yml'))
  ? join(SNIPE_ROOT, 'templates/states.yml')
  : join(SNIPE_ROOT, 'states.yml');
...
// Non-canonical variants accepted on hand-edited rows; normalize-statuses
// folds them into the canonical label on the next run.
const ALIASES = {
  'hold': 'evaluated', 'sent': 'applied', 'monitor': 'skip', 'geo blocker': 'skip',
};
...
for (const e of entries) {
  ...
  if (!match) continue;
  const link = match[1];
  if (!existsSync(join(TRACKER_DIR, link)) && !existsSync(join(SNIPE_ROOT, link))) {
