...
Works in two modes:
- **Slug mode:** `/snipe cover {slug}` — loads the existing evaluation report draft as a starting point
- **Paste mode:** `/snipe cover` or JD pasted directly — starts from scratch
...
When invoked as `/snipe cover {slug}`:
