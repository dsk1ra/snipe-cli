...
```bash
node tracker/followup-cadence.mjs
```
...
> "No active applications to follow up on. Apply to some roles first with `/snipe` and come back when they're aging."
...
- Suggest the user run `/snipe contacto {company}` to find the right person first
...
> - Trying a different contact via `/snipe contacto`
...
**To:** {email or "No contact found — run `/snipe contacto` first"}
...
These defaults can be overridden via `node tracker/followup-cadence.mjs --applied-days N`.
