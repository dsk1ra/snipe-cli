...
Process job URLs stored in `data/pipeline.md`. The user adds URLs at any time and then executes `/snipe pipeline` to process them all.

## Workflow

1. **Read** `data/pipeline.md` → search for `- [ ]` items in the "Pending" section
2. **For each pending URL**:
   a. Claim the next sequential `REPORT_NUM` atomically by running `node tracker/reserve-report-num.mjs` (and release the sentinel using `node tracker/reserve-report-num.mjs --release <num>` after the report is written)
...
1. Run `node tracker/reserve-report-num.mjs` to claim the next sequential number (stdout returns `{###}`).
2. Write the report file using that number.
3. Release the sentinel by running `node tracker/reserve-report-num.mjs --release {###}` once the report is written.
