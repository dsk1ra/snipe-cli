...
5. **Profile** at `config/profile.yml` + `config/profile.md` — read for candidate context
...
- **"Walk me through your CV / why are you looking?"** — 60–90s narrative anchored to `config/profile.md` narrative + the role's archetype.
...
- **Location / remote / visa** — answer derived from `config/profile.yml` location policy and the role's posted policy. Flag deal-breakers from `config/profile.md` so the recruiter can route correctly.
...
- **Background red flags** — gaps, transitions, unusual elements from `cv.md` + `config/profile.md`. Honest, specific, forward-looking framing — never defensive.
...
- **"Why this role, why now?"** — connect candidate's last 1–2 roles + `config/profile.md` narrative to the team's named challenge from Step 1.
...
- **What to lead with**: connection between candidate narrative (`config/profile.md`) and a named team challenge from Step 1.
