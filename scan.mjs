...
const PORTALS_PATH = process.env.SNIPE_PORTALS || 'portals.yml';
...
async function main() {
  ...
  const tasks = targets.map(company => async () => {
    ...
    try {
      ...
      for (const job of jobs) {
        ...
        // Age filter: global max_age_hours in portals.yml, per-entry override
        // (0 disables). Fail-closed: undated postings are dropped too — an
        // unverifiable age counts as too old. Sources that never expose dates
        // need max_age_hours: 0 on their entry.
        const maxAgeH = company.max_age_hours ?? config.max_age_hours;
        if (maxAgeH && (!Number.isFinite(job.postedAt)
            || Date.now() - job.postedAt > maxAgeH * 3_600_000)) {
          totalFilteredAge++;
          continue;
        }
  ...
  console.log(`\n→ Run /snipe pipeline to evaluate new offers.`);
